...
class RenderSliverFillRemainingBoxAdapter extends RenderSliverSingleBoxAdapter {
  ...
  double sizeChildWithMin(double minExtent) {
    child.layout(constraints.asBoxConstraints(minExtent: minExtent),
        parentUsesSize: true);
    double childExtent;
    switch (constraints.axis) {
      case Axis.horizontal:
  ...
  void performLayout() {
    ...
    if (child == null) {
      ...
      if (scrollController.offset < constraints.viewportMainAxisExtent) {
        paintedSize =
            constraints.remainingPaintExtent - scrollController.offset;
      } else {
        paintedSize = 0.0;
      }
    ...
    if (scrollController.offset + childExtent <
        constraints.viewportMainAxisExtent) {
      childExtent = sizeChildWithMin(math.max(childExtent,
          constraints.remainingPaintExtent - scrollController.offset));
    }
